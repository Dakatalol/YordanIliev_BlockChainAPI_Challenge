...
describe('Jupiter Swap API Tests', () => {
  ...
  describe('Input Validation Tests', () => {
    ...
    it('TC-04: Missing Required Fields - Send request without quoteResponse', async () => {
      ...
      expect((response.data as any).error).to.include('Failed to deserialize the JSON body into the target type: missing field `quoteResponse`');
    });
    ...
    it('TC-09: Invalid Quote - Negative amount in quote', async () => {
      ...
      expect(response.status).to.equal(422);
      expect((response.data as any).error).to.include('Failed to deserialize the JSON body into the target type: quoteResponse.inAmount');
    });
    ...
    it('TC-10: Invalid Quote - Same input and output mint', async () => {
      ...
      expect((response.data as any).error).to.equal('Input and output mints are not allowed to be equal');
