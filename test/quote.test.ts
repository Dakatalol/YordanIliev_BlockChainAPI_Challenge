...
describe('Jupiter Quote API Tests', () => {
  ...
  describe('Business Logic Tests', () => {
    ...
    it('TC-08: Should calculate slippage correctly', async () => {
      ...
      QuoteValidators.validateSlippageCalculation(response, VALID_SCENARIOS.SLIPPAGE_CALCULATION_TEST);
    });
  ...
  describe('Large Amount Tests', () => {
    ...
    it('TC-11: Should return multi-hop routes for large SOL amount (1M SOL)', async () => {
      ...
      expect(response.data.routePlan.length).to.be.greaterThan(1, 
  ...
  describe('Input Validation Tests', () => {
    ...
    it('TC-06: Should reject zero amount', async () => {
      ...
      QuoteValidators.validateBadRequest(response, ['Could not find any route'], JUPITER_ERROR_CODES.COULD_NOT_FIND_ANY_ROUTE);
    });
    ...
    it('TC-07: Should reject same input and output mint', async () => {
      ...
      QuoteValidators.validateBadRequest(response, ['Input and output mints are not allowed to be equal'], JUPITER_ERROR_CODES.CIRCULAR_ARBITRAGE_IS_DISABLED);
    });
  });
